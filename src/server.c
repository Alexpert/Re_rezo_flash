...
void send_post_to(Post *p, User *dest, Flash_Instance *f)
{
	...
	if (dest->socket > 0)
	{
		...
		strcpy(msg->cmd_id, "7");
		strcpy(msg->login, getUser_from_id(p->author_id, f)->login);
		strcpy(msg->arg, p->content);
		char *buffer = malloc(sizeof(Message));
		memcpy((void *)buffer, (void *)msg, sizeof(Message));
		h_writes(dest->socket, buffer, sizeof(Message));
...
void send_user_to(User *u, int socket)
{
	Message *msg = malloc(sizeof(Message));
	strcpy(msg->cmd_id, "5");
	strcpy(msg->arg, u->login);
	char *buffer = malloc(sizeof(Message));
	memcpy((void *)buffer, (void *)msg, sizeof(Message));
	h_writes(socket, buffer, sizeof(Message));
}
...
void respond_client(int socket, char *str_recv, struct server_instance *s)
{
	...
	*/
	...
	case '1':
		...
		{
			strcpy(res->cmd_id, "1");
			strcpy(res->arg, "Welcome!");
			printf("login 1\n");
		}
		else
		{
			strcpy(res->cmd_id, "-1");
			strcpy(res->arg, "Login failed.");
			printf("login -1\n");
		}
		break;
	case '2':
		...
		{
			strcpy(res->cmd_id, "2");
			strcpy(res->arg, "Subscribed!");
		}
		else
		{
			strcpy(res->cmd_id, "-2");
			strcpy(res->arg, "Sub. failed.");
		}
		break;
	case '3':
		...
		{
			strcpy(res->cmd_id, "3");
			strcpy(res->arg, "Unscubscribed!");
		}
		else
		{
			strcpy(res->cmd_id, "-3");
			strcpy(res->arg, "Unsub. failed.");
		}
		break;
	case '4':
		printf("Publishing\n");
		...
		{
			strcpy(res->cmd_id, "4");
			strcpy(res->arg, "Published!");
			send_post(p, f);
		}
		else
		{
			strcpy(res->cmd_id, "-4");
			strcpy(res->arg, "Publish failed.");
		}
		break;
	case '5':
	...
	case '6':
		...
		{
			strcpy(res->cmd_id, "6");
			strcpy(res->arg, "See you next time!");
		}
		else
		{
			strcpy(res->cmd_id, "-6");
			strcpy(res->arg, "Logout failed.");
		}
		break;
	}
