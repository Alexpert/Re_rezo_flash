...
User *getUser_from_id(int id, Flash_Instance *f) {
  int i = 0;
  while (i < f->users->current_index && ((User *) get(f->users, i))->id != id)
    ++i;
...
User *getUser_from_login(char *login, Flash_Instance *f) {
  int i = 0;
  while (i < f->users->current_index && strcmp(((User *) get(f->users, i))->login, login))
    ++i;
  if (i == f->users->current_index)
    return NULL;
...
int unsubscribe(char *following, char *follower, Flash_Instance *f) {
  ...
  element_t *e = remove_element(following_struct->followers, follower_struct);
...
int publish(int id, char *message, Flash_Instance *f) {
  ...
  memcpy(post->readers->data, publisher->followers->data, publisher->followers->current_index);
