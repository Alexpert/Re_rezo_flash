...
int h_socket(int domaine, int mode)
{
	...
	if (domaine != AF_INET)
		printf(" Sous internet utiliser -> AF_INET\n");
	if ((mode != SOCK_STREAM) && (mode != SOCK_DGRAM))
		printf(" Mode a choisir parmi : SOCK_STREAM, SOCK_DGRAM \n");
...
void h_connect(int num_soc, struct sockaddr_in *p_adr_serv)
{
	...
	res =
	    connect(num_soc, (struct sockaddr *)p_adr_serv,
		    sizeof(struct sockaddr));

	if (res < 0) {
		inet_ntop(AF_INET, &(p_adr_serv->sin_addr), str,
			  INET_ADDRSTRLEN);
		printf
...
int h_accept(int num_soc, struct sockaddr_in *p_adr_client)
{
	...
	res =
	    accept(num_soc, (struct sockaddr *)p_adr_client,
		   (socklen_t *) & longueur);
	if (res < 0)
	     ...
	     struct sockaddr_in *p_adr_distant)
{
	...
	res =
	    sendto(num_soc, tampon, nb_octets, 0,
	       ...
	       struct sockaddr_in *p_adr_distant)
{
	...
	res =
	    recvfrom(num_soc, tampon, nb_octets, 0,
		     (struct sockaddr *)p_adr_distant, (socklen_t *) & taille);
	if (res < 0)
		printf
